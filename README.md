...
- **一部チームはドライバー未発表**: チームノア・WING HIN MOTORSPORTS JAPAN・
  ...
  ため取得できていない(公式発表され次第、次回更新で自動的に反映される)。WING HIN
  MOTORSPORTS JAPANのみ、2026年参戦体制発表時の報道(autosport web)を情報源とした
  ドライバー名を静的フォールバックとして表示している(公式サイト側が更新され次第、
  実データが優先される)。
- **各ラウンドのエントリーリスト/正式結果PDFはドライバー名の自動抽出に使用していない**:
  supertaikyu.comが公開する決勝正式結果PDF(files/result/2026/配下)は、フォントの
  文字コードマッピングが非標準でテキスト抽出すると文字化けする(pdfplumber等の一般的な
  PDF処理ライブラリでも正しく読み取れないことを確認済み)。目視でも列のズレによる
  誤読リスクが高く、実在の人物名を誤って表示するリスクを避けるため、これらのPDFからの
  自動抽出は行っていない。
