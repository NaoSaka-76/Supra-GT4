...
- **ドライバーはラウンド別エントリーリストの集計**: 当初はチーム個別ページ
  (teams/2026_XXX.html)を情報源にしていたが、一部チームでドライバー名が伏字
  ("※※※※※")のまま更新されないケースがあった。各ラウンドの公式エントリーリスト
  (race/round_XX.html)には確定済みの氏名が掲載されていることを確認したため、
  開幕戦(第1戦)から直近の開催済みラウンドまでの全エントリーリストを集計し、
  重複を除いた「今シーズンここまでに参戦した全ドライバー」を表示する方式に変更した。
  そのラウンドの開催日を過ぎているかどうかで「開催済み」を判定しているため、
  ラウンド間の日程が変則的な場合など、実際の開催状況とずれる可能性はゼロではない。
- **同一人物の表記ゆれは統合していない**: 公式エントリーリスト側の入力ゆれ(例:
  ラウンドによって「FOK WAI MIN」/「FOK WAI MING」のように末尾表記が異なる)は、
  別人と誤って統合してしまうリスクを避けるため、あえて名寄せせず別エントリーとして
  そのまま表示している。
- **決勝正式結果PDFはドライバー名の自動抽出に使用していない**: supertaikyu.comが
  公開する決勝正式結果PDF(files/result/2026/配下)は、フォントの文字コード
  マッピングが非標準でテキスト抽出すると文字化けする(pdfplumber等の一般的なPDF
  処理ライブラリでも正しく読み取れないことを確認済み)。目視でも列のズレによる
  誤読リスクが高く、実在の人物名を誤って表示するリスクを避けるため、これらのPDF
  からの自動抽出は行っていない。
